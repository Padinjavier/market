...
class Ticketcopy extends Controllers {
    ...
    //     $linkimg = media() . "/tienda/images/logo2.png";
    ...
    //     $pdf->Ln( 4/2); // Espacio después de la imagen
    //     $pdf->SetFont('Arial','',8);
    ...
    private function generarPDF($datos_venta) {
        ...
        $pdf = new PDF_Code128('P', 'mm', array(80, 140)); // Ajusta el tamaño del documento
        $pdf->SetMargins(4, 4, 4);
        $pdf->AddPage();
        $pdf->SetFont('Arial','B',9);
        $pdf->SetTextColor(0,0,0);
        ...
        $pdf->SetFont('Arial','B',8);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","S.A."),0,'C',false);
        $pdf->SetFont('Arial','',7);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","MOLLCOPATA NEGOCIOS SRL"),0,'C',false);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","SOLUCIONES, CRECIMIENTO, VIDA"),0,'C',false);
        
        $pdf->Ln(3);
    
        // Información del cliente
        $pdf->SetFont('Arial','',7);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","ROSSANA GUISELA VILLAFUERTE GARCIA - 1015414121"),0,'C',false);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","C.P LA QUEBRADA"),0,'C',false);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","MZ C1 LT 1 - SAN LUIS CAÑETE"),0,'C',false);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","CEL: 975125285 - 920056236"),0,'C',false);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","1.4-QUILMANA - V_3112"),0,'C',false);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1",$datos_venta['venta_fecha_hora']),0,'C',false);
    
        $pdf->Ln(3);
    
        // Detalles del recibo
        $pdf->SetFont('Arial','',8);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","TIPO DE PAGO:  CONTADO / EFECTIVO"),0,'L',false);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","ESTADO DEL    ENTREGADO"),0,'L',false);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","PRODUCTO:"),0,'L',false);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","TIPO MONEDA:   SOLES"),0,'L',false);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","CLIENTE:  DEMETRIO IPURRE MALDONADO"),0,'L',false);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","VENDEDOR: GLORIA CECILIA PADIN SALDAÑA"),0,'L',false);
        
        $pdf->Ln(3);
        
        // Tabla de productos
        $pdf->SetFont('Arial','B',8);
        $pdf->Cell(20,5,"PROD",0,0,'L');
        $pdf->Cell(20,5,"CANT",0,0,'L');
        $pdf->Cell(20,5,"TOTAL",0,1,'L');
        
        $pdf->SetFont('Arial','',8);
        $pdf->Cell(20,5,"SUPER-A 450 X 1LT",0,0,'L');
        $pdf->Cell(20,5,"1.00",0,0,'L');
        $pdf->Cell(20,5,"98.00",0,1,'L');
        
        $pdf->Cell(20,5,"FIPROKILL X 1LT",0,0,'L');
        $pdf->Cell(20,5,"1.00",0,0,'L');
        $pdf->Cell(20,5,"125.00",0,1,'L');
        ...
        $pdf->Cell(30,5,"S/. 182.88",0,1,'R');
        $pdf->Cell(40,5,"OP. GRAVADA:",0,0,'R');
        $pdf->Cell(30,5,"S/. 0.00",0,1,'R');
        $pdf->Cell(40,5,"IGV:",0,0,'R');
        $pdf->Cell(30,5,"S/. 0.00",0,1,'R');
        $pdf->Cell(40,5,"IMPORTE TOTAL:",0,0,'R');
        $pdf->Cell(30,5,"S/. 223.00",0,1,'R');
        $pdf->Cell(40,5,"DESCUENTO:",0,0,'R');
        $pdf->Cell(30,5,"S/. 0.00",0,1,'R');
        $pdf->Cell(40,5,"TOTAL A PAGAR:",0,0,'R');
        $pdf->Cell(30,5,"S/. 223.00",0,1,'R');
